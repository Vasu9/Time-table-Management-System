...
while ($db_field = mysql_fetch_assoc($result2)) {
	...
	if($i=='admin'){
		...
		print("<script>location.href = 'edit_group.php?key=".$namekey."'</script>");
		break;
	}
}	
...
while ($db_field = mysql_fetch_assoc($result)) {
	...
	echo  '<a href="delete_group.php?key='.$h.'" class="btn btn-warning right">Delete Group</a>';
	echo  '<a href="domain_group_message.php?key='.$h.'" class="btn btn-warning right">Message(For sending message to whole group)</a>';
	...
	while ($db_field = mysql_fetch_assoc($result1)) {
		...
		$g = $db_field['doj'];
		echo '<tr>';
		echo '<td><a href="viewuser.php?key='.$d.'">'.$d.'</a></td>';
		echo '<td>'.$f.'</td>';
		echo '<td>'.$g.'</td>';
		echo '</tr>';
